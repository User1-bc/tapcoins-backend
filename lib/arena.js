...
const { cardStats, hash, mulberry32, roleOf, deckCombos } = require('./cards');
...
const MAX_ROUNDS = 30;
const FATIGA_START = 16;   // desde esta ronda todos pierden vida (evita empates longos)
const FATIGA = 0.04;
const DECK_SIZE = 4;
...
function makeBotDeck(seed) {
  ...
  const roleCount = {};
  let guard = 0;
  while (ids.size < DECK_SIZE && guard++ < 500) {
    const id = 1 + Math.floor(rnd() * 5000);
    if (ids.has(id)) continue;
    const role = roleOf(id);
    if ((roleCount[role] || 0) >= 2) continue; // mazo del bot: máximo 2 del mismo rol
    roleCount[role] = (roleCount[role] || 0) + 1;
    ids.add(id);
  }
  while (ids.size < DECK_SIZE) ids.add(1 + Math.floor(rnd() * 5000));
...
function attachArena(server, ctx) {
  ...
  function createBattle(a, b, opts) {
    ...
    const rawA = a.isBot ? a.deck : a.deck;
    const rawB = b.isBot ? b.deck : b.deck;
    // Sinergias de mazo (Alivio, Espinas, Vinculo, Afinidad) aplicadas a los
    // stats finales: el cliente ve exactamente lo mismo que simula el servidor.
    const combos = [deckCombos(rawA.map((c) => c.stats)), deckCombos(rawB.map((c) => c.stats))];
    const teamA = effectiveTeam(rawA, combos[0]);
    const teamB = effectiveTeam(rawB, combos[1]);
    const state = {
      id,
      kind: opts && opts.kind,
      players: [a.pid, b.pid],
    ...
    a.battleId = id;
    b.battleId = id;
    const teams = [serializeTeam(teamA), serializeTeam(teamB)];
    if (!a.isBot) send(a.ws, { t: 'matched', battleId: id, you: 0, seed, combos, teams });
    if (!b.isBot) send(b.ws, { t: 'matched', battleId: id, you: 1, seed, combos, teams });
    scheduleRound(state);
  }

  function effectiveTeam(deck, combos) {
    const affinity = (combos.find((c) => c.startsWith('Afinidad:')) || '').split(':')[1] || '';
    const hasThorns = combos.includes('Espinas');
    return deck.map((c) => {
      const s = Object.assign({}, c.stats);
      if (affinity && s.element === affinity) s.atk = Math.round(s.atk * 1.12);
      if (!hasThorns) s.thorns = 0;
      const out = { cardId: c.cardId };
      if (c.serial) out.serial = c.serial;
      out.stats = s;
      return out;
    });
  }
  ...
  function resolveRound(state) {
    ...
    if (aliveCount(state, 0) > 0 && aliveCount(state, 1) > 0) endOfRound(state);
  ...
  function applyAction(state, side, pick) {
    const opp = 1 - side;
    const ai = pick.attacker;
    const ti = pick.target;
    if (!state.alive[side][ai] || !state.alive[opp][ti]) return;
    const card = state.teams[side][ai];
    const st = card.stats;
    const roll = state.rng();
    let atk = st.atk;
    if (st.proud) {
      const ratio = state.hp[side][ai] / st.hp;
      atk *= ratio > 0.6 ? 1.12 : (ratio < 0.3 ? 0.8 : 1);
    }
    let dmg = atk * (0.8 + 0.4 * roll);
    let crit = false;
    if (state.rng() < st.crit / 100) {
      dmg *= 2;
      crit = true;
    }
    dmg = Math.max(1, Math.round(dmg * state.teams[opp][ti].stats.dr));
    state.hp[opp][ti] = Math.max(0, state.hp[opp][ti] - dmg);
    let line = `${shortName(card)} ataca a ${shortName(state.teams[opp][ti])} por ${dmg}`;
    if (crit) line += ' ¡CRÍTICO!';
    state.log.push(line);
    checkDeath(state, opp, ti);

    // Sangrante: roba vida del daño que inflige
    if (st.lifesteal > 0 && state.alive[side][ai] && state.hp[side][ai] > 0) {
      const heal = Math.min(Math.round(dmg * st.lifesteal), st.hp - state.hp[side][ai]);
      if (heal > 0) {
        state.hp[side][ai] += heal;
        state.log.push(`${shortName(card)} absorbe ${heal} de vida 🩸`);
      }
    }

    // Espinas: el tanque devuelve una parte del daño
    const thorns = state.teams[opp][ti].stats.thorns;
    if (thorns > 0 && state.alive[side][ai]) {
      const back = Math.max(1, Math.round(dmg * thorns));
      state.log.push(`${shortName(state.teams[opp][ti])} devuelve ${back} con espinas 🛡️`);
      state.hp[side][ai] = Math.max(0, state.hp[side][ai] - back);
      checkDeath(state, side, ai);
    }
  }

  function checkDeath(state, side, idx) {
    if (state.hp[side][idx] > 0) return;
    const card = state.teams[side][idx];
    if (cardSidesRevive(state, side, idx)) {
      state.revive[side][idx] = 1;
      state.hp[side][idx] = Math.floor(card.stats.hp / 2);
      state.log.push(`${shortName(card)} ¡resucita con el 50% de vida!`);
    } else {
      state.alive[side][idx] = false;
      state.log.push(`${shortName(card)} cae fuera de combate 😵`);
    }
  }

  // Curas, regeneración y fatiga al final de cada ronda.
  function endOfRound(state) {
    const fatiga = state.round >= FATIGA_START;
    if (fatiga && state.round === FATIGA_START) {
      state.log.push('La fatiga empieza a castigar a los equipos 😩');
    }
    for (let s = 0; s < 2; s++) {
      // Vital: 5% del HP máximo
      for (let i = 0; i < 4; i++) {
        const st = state.teams[s][i].stats;
        if (!state.alive[s][i] || st.regen <= 0 || state.hp[s][i] <= 0) continue;
        if (state.hp[s][i] >= st.hp) continue;
        const heal = Math.max(1, Math.round(st.hp * st.regen));
        state.hp[s][i] = Math.min(st.hp, state.hp[s][i] + heal);
        state.log.push(`${shortName(state.teams[s][i])} regenera ${heal} de vida ✨`);
      }
      // Alivio: el curandero cura al aliado más herido
      if (state.combos[s].includes('Alivio')) {
        const healer = state.teams[s].findIndex((c) => c.stats.role === 'Curacion' && state.alive[s][indexOfCard(state, s, c)]);
        if (healer >= 0) {
          let worst = -1;
          let worstRatio = 1.01;
          for (let i = 0; i < 4; i++) {
            if (!state.alive[s][i] || state.hp[s][i] <= 0) continue;
            const ratio = state.hp[s][i] / state.teams[s][i].stats.hp;
            if (ratio < worstRatio) {
              worstRatio = ratio;
              worst = i;
            }
          ...
          if (worst >= 0 && worstRatio < 1) {
            const st = state.teams[s][worst].stats;
            const heal = Math.min(Math.round(st.hp * 0.09), st.hp - state.hp[s][worst]);
            if (heal > 0) {
              state.hp[s][worst] += heal;
              state.log.push(`Alivio: ${shortName(state.teams[s][worst])} recupera ${heal} de vida 💚`);
            }
          }
        }
      }
      // Vinculo: una vez por batalla, cura a un aliado crítico
      if (!state.vinculoUsed[s] && state.combos[s].includes('Vinculo')) {
        for (let i = 0; i < 4; i++) {
          if (!state.alive[s][i] || state.hp[s][i] <= 0) continue;
          const st = state.teams[s][i].stats;
          if (state.hp[s][i] / st.hp >= 0.25) continue;
          const heal = Math.min(Math.round(st.hp * 0.30), st.hp - state.hp[s][i]);
          state.vinculoUsed[s] = true;
          if (heal > 0) {
            state.hp[s][i] += heal;
            state.log.push(`Vinculo: ${shortName(state.teams[s][i])} salva la vida con ${heal} ✨`);
          }
          break;
        }
      }
      // Fatiga: desgaste que fuerza a resolver la batalla
      if (fatiga) {
        for (let i = 0; i < 4; i++) {
          const st = state.teams[s][i].stats;
          if (!state.alive[s][i] || state.hp[s][i] <= 0) continue;
          state.hp[s][i] = Math.max(1, state.hp[s][i] - Math.max(1, Math.round(st.hp * FATIGA)));
        }
      }
    }
  }

  function indexOfCard(state, side, card) {
    return state.teams[side].indexOf(card);
  }
